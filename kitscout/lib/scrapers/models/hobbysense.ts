...
export async function scrapeHobby(query: string): Promise<KitResult[]> {
    
    //this is the JSON endpoint
    const url = `https://hobbysense.ca/search/suggest.json?q=${encodeURIComponent(query)}`;
    ...
    const products = data?.resources?.results?.products ?? [];
     ...
     for (const p of products) {
        ...
        const currency = "CAD";
        const link = p.url.startsWith("http") ? p.url : `https://hobbysense.ca${p.url}`;
        const image = p.image || p.featured_image?.url;
        const source = "Hobby Sense";
